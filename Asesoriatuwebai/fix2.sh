...
echo "Aplicando solución simplificada al chatbot (versión 2)..."
...
# Verificar si existe websy-image.jpg y copiarlo
if [ -f "websy-image.jpg" ]; then
  echo "Copiando avatar desde websy-image.jpg..."
  ...
  echo "Advertencia: No se encontró websy-image.jpg"
fi

# Corrección de vite.config.ts 
echo "Actualizando vite.config.ts..."
cat > tuwebaiext/vite.config.ts << 'EOL'
...
# Verificar si existe el directorio del chatbot
if [ -d "tuwebaiext/client/src/components/ui" ]; then
  echo "Actualizando componente de chatbot..."
  
  # Crear chatbot mejorado
  cat > tuwebaiext/client/src/components/ui/chatbot.tsx << 'EOL'
...
EOL
  echo "✓ Componente de chatbot actualizado"
else
  echo "Error: No se encontró el directorio del chatbot"
fi
